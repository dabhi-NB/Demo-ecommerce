...
function createLimiter(
  options: { keyPrefix?: string; max?: number; windowSeconds?: number } = {}
): RequestHandler {
  const { keyPrefix = "rl", max = 255, windowSeconds = 5 * 60 } = options;
...
if (config.CACHE_DRIVER && config.CACHE_DRIVER.toLowerCase() === "redis") {
  ...
  forgotPwdLimiter = createLimiter({
  ...
  generalLimiter = expressRateLimit({
    windowMs: 15 * 60 * 1000,
    max: 100,
    message: {
      status: 0,
