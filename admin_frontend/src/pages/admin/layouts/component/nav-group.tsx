...
export function NavGroup({ title, items }: NavGroupProps) {
  ...
  // Use useMemo to recalculate filtered items when user permissions change
  const filteredItems = useMemo(() => {
    return items
      ...
      .map((item) => {
        ...
        if (item.items && Array.isArray(item.items)) {
          const filteredSubItems = item.items.filter((subItem) => {
            // If no permission is required, show the sub-item
            if (
      ...
      .filter((item) => {
        // Remove items that have no sub-items if they originally had sub-items
        if (
          item.items &&
  ...
  }, [items, hasPermission, user?.permission]);
