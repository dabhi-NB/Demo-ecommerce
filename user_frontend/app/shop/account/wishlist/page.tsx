...
export default function WishlistPage() {
  ...
  const { wishlistProducts, isLoading, clearWishlist, totalWishlist } =
    useWishlist();
  ...
  useEffect(() => {
    if (!isAuthenticated)
      router.replace("/auth/login?redirect=/shop/account/wishlist");
  }, [isAuthenticated]);

  // Redirect if wishlist feature disabled by admin
  useEffect(() => {
  ...
  return (
    <AccountLayout
      title="My Wishlist"
      subtitle={`${totalWishlist} item${totalWishlist !== 1 ? "s" : ""} saved`}
    >
      {/* EMPTY STATE */}
